...
//                         console.log(inputFiles.get('testname1'));
...
// console.log( inputFiles.getAll('file')?inputFiles.getAll('file').length:'length null');
                        ...
                        return (
                            ...
                            <div>
                                {(inputFiles.getAll('file').length)?inputFiles.getAll('file').map(function(file){
                                    return <div key={file.name} className="to-m-top-8">{file.name}</div>
                                }):<span>可以把文件拖到这里</span>}
                            </div>
                        )
                        }
